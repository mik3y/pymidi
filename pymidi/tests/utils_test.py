...
class UtilsTests(TestCase):
    def test_is_ipv4_address(self):
        ...
        for ip in bad_ips:
            self.assertEqual(
                False, utils.is_ipv4_address(ip), 'Expected {} to return False'.format(repr(ip))
            )

        good_ips = ['127.0.0.1', '8.8.8.8']
        for ip in good_ips:
            self.assertEqual(
                True, utils.is_ipv4_address(ip), 'Expected {} to return True'.format(repr(ip))
            )
    ...
    def test_is_ipv6_address(self):
        ...
        for ip in bad_ips:
            self.assertEqual(
                False, utils.is_ipv6_address(ip), 'Expected {} to return False'.format(repr(ip))
            )

        good_ips = ['::', '2001:0db8:85a3:0000:0000:8a2e:0370:7334']
        for ip in good_ips:
            self.assertEqual(
                True, utils.is_ipv6_address(ip), 'Expected {} to return True'.format(repr(ip))
            )
